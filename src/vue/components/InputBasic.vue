  ...
  <div class="field-group">
    ...
    <div class="field">
      <label for="monthlyContribution">
        月額積立（万円）
        <HelpIcon text="毎月この額（万円）を積み立てる。NISA年間枠・生涯枠の範囲内で優先的にNISAへ、超過分は特定（リスク）へ。積立年数の間だけ実行され、切崩開始後と重なる期間も継続する。防衛資産には積み立てない。積立分は含み益ゼロ（取得価額=積立額）として扱う。" />
      </label>
      <InputNumber id="monthlyContribution" v-model="state.monthlyContributionMan" min="0" step="1" />
    </div>
  </div>
</template>
