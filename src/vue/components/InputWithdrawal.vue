...
<template>
  <div class="field-group">
    ...
    <template v-if="isRateMode">
      <div class="field">
        <label for="withdrawalRate">
          年間引出率（%）
          <HelpIcon text="年あたりの引出割合。Trinity Study モードでは取り崩し開始時の総資産に対する率（4%が目安）。リスク資産モードでは毎年初時点のリスク資産に対する率。「自動算出」ボタンで、成功率95%（取り崩し終了時に資産が枯渇しない確率）を満たす最大の引出率を二分探索で求められます。なお Trinity Study の 4% は米国市場・税ゼロ前提の値。国内課税・為替リスクを考慮すると 3〜3.5% スタートが目安とされる。" />
        </label>
        <div class="rate-with-button">
    ...
    <div v-if="showLimitTable" class="field field--full">
      <label>
        月額下限・上限（年齢ステップ式）
        <HelpIcon text="年齢に応じて月額下限・上限を段階的に変えられます。各行の「〜歳まで」までその区間の下限・上限を適用、最後の行は「以降ずっと」。値は「現在の購買力（万円）」で、シミュレーション内では自動でインフレ名目化されます。空欄は無制限。下限>上限のときは上限が優先されます。Guyton-Klinger モードでは GK 調整後の引出額をさらにこの下限・上限で挟むため、「最低限この金額は引き出す／この金額以上は引き出さない」という生活費総額（年金等の収入を含む合計）の安全網として活用できます。例：下限 30 万・年金 15 万 → 資産取り崩し 15 万・生活費合計 30 万。" />
      </label>
