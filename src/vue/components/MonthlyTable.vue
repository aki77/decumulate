  ...
  <template v-for="g in yearGroups" :key="g.year">
    <details class="monthly-year">
      ...
      <table class="monthly-table">
        ...
        <tbody>
          <tr v-for="r in g.rows" :key="r.month">
            <td>{{ r.month }}月</td>
            <td>
              {{ formatMan(r.nisaTotal) }}
              <span v-if="r.total > 0" class="cell-sub">({{ formatPercent(r.nisaTotal / r.total) }})</span>
            </td>
            <td>
              {{ formatMan(r.taxableRiskTotal) }}
              <span v-if="r.total > 0" class="cell-sub">({{ formatPercent(r.taxableRiskTotal / r.total) }})</span>
            </td>
            <td>
              {{ formatMan(r.defenseTotal) }}
              <span v-if="r.total > 0" class="cell-sub">({{ formatPercent(r.defenseTotal / r.total) }})</span>
            </td>
            <td>
              {{ formatMan(r.idecoTotal) }}
              <span v-if="r.total > 0" class="cell-sub">({{ formatPercent(r.idecoTotal / r.total) }})</span>
            </td>
            ...
            <td class="monthly-rate" :data-rate-band="classifyRateBand(r.monthlyRate, false)">{{ formatRate(r.monthlyRate) }}</td>
            <td>
              <template v-if="r.nisaTransferInfo">
                <span class="transfer-badge" tabindex="0" aria-label="NISA振替">▲<span class="rebalance-tip" v-html="`特定 → NISA 振替<br>売却額 ${formatMan(r.nisaTransferInfo.sellAmount)}<br>税額 ${formatMan(r.nisaTransferInfo.taxAmount)}<br>NISA買付 ${formatMan(r.nisaTransferInfo.proceeds)}`"></span></span>
              </template>
              <template v-if="r.rebalanceInfo">
                <span class="rebalance-badge" :data-direction="r.rebalanceInfo.direction" tabindex="0" :aria-label="r.rebalanceInfo.direction === 'risk-to-defense' ? 'リスク → 防衛' : '防衛 → リスク'">●<span class="rebalance-tip" v-html="`${r.rebalanceInfo.direction === 'risk-to-defense' ? 'リスク → 防衛' : '防衛 → リスク'}<br>売却額 ${formatMan(r.rebalanceInfo.sellAmount)}<br>税額 ${formatMan(r.rebalanceInfo.taxAmount)}<br>受取額 ${formatMan(r.rebalanceInfo.proceeds)}${r.rebalanceInfo.nisaUsed > 0 ? '<br>うちNISA枠充当 ' + formatMan(r.rebalanceInfo.nisaUsed) : ''}`"></span></span>
              </template>
              <template v-if="r.idecoLumpSumInfo">
                <span class="ideco-lump-badge" tabindex="0" aria-label="iDeCo一時金">■<span class="rebalance-tip" v-html="`iDeCo 一時金受取<br>受取総額 ${formatMan(r.idecoLumpSumInfo.grossAmount)}<br>税額 ${formatMan(r.idecoLumpSumInfo.taxAmount)}<br>特定リスクへ ${formatMan(r.idecoLumpSumInfo.proceeds)}`"></span></span>
              </template>
              <template v-if="r.idecoPensionInfo">
                <span class="ideco-pension-badge" tabindex="0" aria-label="iDeCo年金">◆<span class="rebalance-tip" v-html="`iDeCo 年金受取<br>受取総額 ${formatMan(r.idecoPensionInfo.grossAmount)}<br>税額 ${formatMan(r.idecoPensionInfo.taxAmount)}<br>税引後 ${formatMan(r.idecoPensionInfo.proceeds)}`"></span></span>
              </template>
            </td>
          </tr>
        </tbody>
