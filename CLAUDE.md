...
- **Merton ジャンプ拡散（JD）**: `enableJumpDiffusion=true` のとき `zRisk` 消費直後に Knuth 法 Poisson サンプリングで急落を追加する。固定定数（`JD_LAMBDA_PER_YEAR=0.02` 等）はファイル冒頭で宣言。防衛資産には適用しない。RNG 追加消費はジャンプ有り（≈0.17%/月）のときのみ発生するため phase1/phase2 整合性は `enableJumpDiffusion` の値が同一なら自動保証される
