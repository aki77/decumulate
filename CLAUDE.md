...
```sh
npm test            # node:test、--experimental-strip-types で .ts を直接実行（144 件、~4秒）
npm run check       # tsc --noEmit（型チェックのみ）
npm run build       # tsc && dist/ に index.html, styles.css をコピー
npm run dev         # build 後 npx serve dist
```
...
- `localStorage` のスキーマ変更は遠慮なく `STORAGE_KEY` のバージョンを上げて旧データを破棄する
- フィールドの撤去・改名・意味変更は躊躇しない
- マイグレーション層や `if (oldFormat)` 分岐は書かない
- 「念のため残す」「将来の拡張に備える」コードは書かない（YAGNI）

ユーザに「localStorage をクリアしてください」と伝えれば済む。
...
1. `PERSIST_IDS` 配列に入力ID追加（localStorage 永続化）
2. `readParams()` で値を読み取り `CalculateParams`/`MonteCarloParams` に詰める
3. 既存フィールドの**意味を変える**変更なら `STORAGE_KEY` のバージョンを `v3` に上げる（旧データ破棄）
4. `index.html` に対応する `<input>` を追加

表示メトリクスを増やすときは `HELP` 辞書にツールチップ文も追加。
