...
test("rate-guardrail: 初年度は withdrawalRate × リスクサイド / 12 で初期化", () => {
  ...
  assert.ok(Math.abs(firstMonth.monthlyWithdrawal - 1_000_000 * 0.04 / 12) < 1);
});

test("rate-guardrail: 上ガードレール抵触で翌年初に -10% 調整", () => {
  // 引出率 30%: 1年後残高 ≈ 700,000 → 翌年率 ≈ 42.9%（閾値 36% 超 → -10% 発動）
  const initial = 1_000_000;
  const params: CalculateParams = {
    ...BASE_PARAMS,
    ...nisaOnly(initial),
  ...
  const result = calculateCompound(params);
  const firstWithdrawal = result.monthly[0]!.monthlyWithdrawal;
  const secondYearWithdrawal = result.monthly[12]!.monthlyWithdrawal;
  assert.ok(secondYearWithdrawal < firstWithdrawal * 0.95, "上ガードレール抵触で引出額が減るべき");
});

test("rate-guardrail: ガードレール非抵触ではインフレ調整のみ", () => {
  // 運用 = インフレ = 2%（残高は実質横ばい）→ ガードレール非発動
  const initial = 1_000_000;
  const params: CalculateParams = {
    ...BASE_PARAMS,
    ...nisaOnly(initial),
  ...
  const firstWithdrawal = result.monthly[0]!.monthlyWithdrawal;
  const secondYearWithdrawal = result.monthly[12]!.monthlyWithdrawal;
  // インフレ調整のみなら 2年目は 約 1.02 倍になるはず
  const expected = firstWithdrawal * 1.02;
  assert.ok(
    Math.abs(secondYearWithdrawal - expected) < firstWithdrawal * 0.05,
    `ガードレール非抵触: 2年目 ${secondYearWithdrawal} vs expected ~${expected}`,
  );
});

test("rate-guardrail: GK調整後の引出額が ceiling を超えないこと（上限が勝つ）", () => {
  const initial = 1_000_000;
  const ceilingAmount = 2_000;
  const params: CalculateParams = {
    ...BASE_PARAMS,
    ...nisaOnly(initial),
    withdrawalMode: "rate-guardrail",
    withdrawalRate: 4,
    withdrawalLimitSchedule: limit(null, ceilingAmount),
    guardrailUpperPercent: 20,
    guardrailLowerPercent: 20,
    guardrailAdjustmentPercent: 10,
    withdrawalYears: 5,
  ...
  const result = calculateCompound(params);
  for (let i = 0; i < Math.min(result.monthly.length, 60); i++) {
    const w = result.monthly[i]!.monthlyWithdrawal;
    assert.ok(w <= ceilingAmount + 1, `month ${i}: withdrawal ${w} > ceiling ${ceilingAmount}`);
  }
});

test("zero-landing 動的取り崩し（決定論版）: zeroLandingCurve あり・資産が増えると baseWithdrawal も増える", () => {
  const goGoMonthly = 200_000;
  const curve = { slowGoStartAge: 75, noGoStartAge: 85, slowGoCoef: 0.8, noGoMonthly: 100_000 };
  const params: CalculateParams = {
    ...BASE_PARAMS,
    ...
    withdrawalLimitSchedule: [{ untilAge: null, floor: 50_000, ceiling: 400_000 }],
    zeroLandingCurve: curve,
  };
  const result = calculateCompound(params);
  // 高リターン（10%）で資産が増えるので baseWithdrawal も goGoMonthly より大きくなるはず
  const latestRows = result.monthly.slice(60, 72);
  const avgBase = latestRows.reduce((s, r) => s + r.baseWithdrawal, 0) / latestRows.length;
  assert.ok(
    avgBase > goGoMonthly,
    `高リターンで資産増: avgBase=${Math.round(avgBase)} が基準 ${goGoMonthly} を超えるはず`,
  );
});

test("zero-landing 動的取り崩し（決定論版）: No-Go 期は noGoMonthly 固定になる", () => {
  const noGoMonthly = 100_000;
  const curve = { slowGoStartAge: 75, noGoStartAge: 85, slowGoCoef: 0.8, noGoMonthly };
  const params: CalculateParams = {
    ...BASE_PARAMS,
    currentAge: 60,
    initialNisa: 50_000_000,
  ...
  const result = calculateCompound(params);
  // No-Go 期（85歳以降 = 25年目以降）の月次行を検証
  const noGoRows = result.monthly.filter((r) => r.age !== null && r.age >= 85);
  assert.ok(noGoRows.length > 0, "No-Go 期の月次行が存在する");
  for (const row of noGoRows) {
    assert.ok(
      Math.abs(row.baseWithdrawal - noGoMonthly) <= 1,
