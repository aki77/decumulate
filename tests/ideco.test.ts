...
test("stepIdeco - 受取開始月に一時金一括（lumpSumRatio=1, 控除内なら税ゼロ）", () => {
  ...
  const r = stepIdeco(s0, ideco, 1, 0, 1, 65, 0);
...
test("calculateCompound - iDeCo一時金は受取月に特定リスクへ振替", () => {
  ...
  // year=5 末では受取済み（iDeCo残高ゼロ、特定リスクへ振替済み）
  const y5 = r.yearly[5]!;
  assert.strictEqual(y5.idecoTotal, 0);
...
test("回帰: currentAge=60, idecoReceiveStartAge=65 のとき age=65 の1月に一時金発火（age=66 ではない）", () => {
  const params: CalculateParams = {
    ...BASE_PARAMS,
    currentAge: 60,
    contributionYears: 0,
  ...
  const r = calculateCompound(params);
  const m65 = r.monthly.find((x) => x.age === 65 && x.month === 1);
  assert.ok(m65?.idecoLumpSumInfo != null, "age=65の1月に一時金イベントがあること");
  const m66 = r.monthly.find((x) => x.age === 66 && x.month === 1);
  assert.ok(m66?.idecoLumpSumInfo == null, "age=66の1月に一時金イベントがないこと");
});
