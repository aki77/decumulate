...
test("simulateMonteCarlo - シード固定で再現性あり", () => {
  const r1 = simulateMonteCarlo(BASE_PARAMS, SEED);
  const r2 = simulateMonteCarlo(BASE_PARAMS, SEED);
...
test("simulateMonteCarlo (rate) - シード固定で下限/上限指定時も再現性あり", () => {
  ...
  const a = simulateMonteCarlo(params, SEED);
  const b = simulateMonteCarlo(params, SEED);
...
test("simulateMonteCarlo - pivotMonthlies の baseWithdrawal は年金控除前の生活費目標額", () => {
  const params: MonteCarloParams = {
    ...BASE_PARAMS,
    ...withBuckets(100_000_000),
    currentAge: 30,
    withdrawalStartYear: 30,
    withdrawalYears: 30,
    fixedMonthlyWithdrawal: 250000,
    inflationAdjustedWithdrawal: false,
    basePension: 150000,
    pensionStartAge: 65,
    volatility: 0,
  ...
  // 年金開始後（65歳以降 = year 36以降）の行を検証
  const postPensionRows = p50.filter((r) => r.age !== null && r.age >= 65 && r.monthlyPension > 0);
  assert.ok(postPensionRows.length > 0, "年金受給行が存在する");

  for (const row of postPensionRows) {
    // baseWithdrawal = 生活費目標（年金控除前） = fixedMonthlyWithdrawal = 25万
    assert.ok(
      Math.abs(row.baseWithdrawal - 250000) <= 1,
      `baseWithdrawal=${row.baseWithdrawal} 期待: 250000 (year=${row.year} age=${row.age})`,
    );
    // 実取り崩し額 = baseWithdrawal - pension
    assert.ok(
      Math.abs(row.monthlyWithdrawal - (row.baseWithdrawal - row.monthlyPension)) <= 1,
      `monthlyWithdrawal=${row.monthlyWithdrawal} 期待: ${row.baseWithdrawal - row.monthlyPension} (year=${row.year})`,
    );
  }
});
