...
// --- v8→v9 マイグレーション（lifeEvents 追加）---

test("migrateWith - v8→v9: lifeEvents が補完される", () => {
  const v8Data = {
    currentAge: 50,
    initialNisaMan: 1000,
    otherIncomes: [],
  ...
  const result = migrateWith({ version: 8, data: v8Data }, MIGRATIONS, CURRENT_VERSION);
  assert.ok(result);
  assert.deepStrictEqual(result.lifeEvents, []);
});

test("parseStoredState - lifeEvents 配列を読み取る", () => {
  const data = {
    ...validV8Data,
    lifeEvents: [
      { id: "1", label: "車買い替え", amountMan: 200, age: 55 },
    ],
  };
  const result = parseStoredState(data);
  assert.ok(result?.lifeEvents);
  assert.strictEqual(result.lifeEvents!.length, 1);
  assert.strictEqual(result.lifeEvents![0]!.label, "車買い替え");
  assert.strictEqual(result.lifeEvents![0]!.amountMan, 200);
  assert.strictEqual(result.lifeEvents![0]!.age, 55);
});

test("parseStoredState - lifeEvents の破損データは弾く", () => {
  const data = {
    ...validV8Data,
    lifeEvents: [
      { id: "1", label: "valid", amountMan: 200, age: 55 },
      { id: "2", label: "invalid" },
      { label: "no-id", amountMan: 100, age: 50 },
    ],
  };
  const result = parseStoredState(data);
  assert.ok(result?.lifeEvents);
  assert.strictEqual(result.lifeEvents!.length, 1);
  assert.strictEqual(result.lifeEvents![0]!.label, "valid");
});
